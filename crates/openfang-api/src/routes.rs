...
/// Strip frontend-generated `[File: ...]` / `【File: ...】` tags from a message
/// and append `[FILE_ATTACHMENT]` text from hand-agent content blocks directly
/// into the message text.
///
/// This ensures MCP tools (which only see plain text, not content blocks) can
/// read file attachment information. Used by all three message entry points:
/// REST `send_message`, REST `send_message_stream`, and WebSocket `ws.rs`.
///
/// Returns the modified message text. The caller should pass `None` for
/// `content_blocks` to the kernel when using this function.
pub fn inject_hand_attachments_into_message(
    message: &str,
    file_blocks: &[openfang_types::message::ContentBlock],
) -> String {
    ...
    // Strip [File: ...] (half-width brackets)
    ...
    // Strip 【File: ...】 (full-width brackets)
    while let Some(start) = cleaned.find("【File:") {
        if let Some(end) = cleaned[start..].find('】') {
    ...
    let result = format!("{}\n{}", cleaned.trim(), attachment_texts.join("\n"));
    tracing::info!(
        original = %message,
        result = %result,
        "inject_hand_attachments_into_message: final message"
    );
    result
}
    ...
    let content_blocks = if !req.attachments.is_empty() {
        ...
        if is_hand {
            ...
            if !file_blocks.is_empty() {
                message_text = inject_hand_attachments_into_message(&message_text, &file_blocks);
            }
            None
        } else {
...
pub async fn send_message_stream(
    ...
    let stream_content_blocks = if !req.attachments.is_empty() {
        ...
        if is_hand {
            ...
            if !file_blocks.is_empty() {
                stream_message_text = inject_hand_attachments_into_message(&stream_message_text, &file_blocks);
            }
            None
        } else {
            let image_blocks = resolve_attachments(&req.attachments);
